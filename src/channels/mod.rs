...
//!
//! Construct: [`stdio::attach`] / [`web::attach`] / [`matrix::attach`] register
//! `Arc<dyn Channel>` via `Harness::attach`; `Store` mints `ChannelId` and the
//! adapter holds it in its `OnceLock`.
...
//! | [`matrix`] | queued, sent into the direct room | homeserver sync → `Harness::receive` |
//!
